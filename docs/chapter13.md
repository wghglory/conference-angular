...
> note: normally, http return Observable, there must be somewhere call subscribe() to return real data. If we don't need response data, probably call subscribe() immediately after the http. Otherwise, call subscribe() when you call the service Exception: when using resolver, subscribe is not needed (Event-list and event-detail)
...
delete event-route-activator.ts guard since we don't want to call http request twice, also delete any related code in module, route.
